...
class AdminAddStoryScreen extends ConsumerStatefulWidget {
  const AdminAddStoryScreen({super.key});

  @override
  ConsumerState<AdminAddStoryScreen> createState() => _AdminAddStoryScreenState();
}

class _AdminAddStoryScreenState extends ConsumerState<AdminAddStoryScreen> {
  // --- PRO COLORS ---
  ...
  Future<void> _saveStory() async {
    ...
    final newStory = StoryModel(
      ...
      // likes: 0, dislikes: 0, views: 0, // Removed if not present in Admin Model constructor or add if needed
      // adminId & id filled by service
    );

    await ref.read(adminMultimediaViewModelProvider.notifier).addStory(newStory);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Create Global Story", style: GoogleFonts.poppins(color: _textDark, fontWeight: FontWeight.w700, fontSize: 17.sp)),
        centerTitle: true,
      ),
      body: Stack(
        children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        final newPage = StoryPage(text: textCtrl.text, imageUrl: pageImage?.path ?? "");
