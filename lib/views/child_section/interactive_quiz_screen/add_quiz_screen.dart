...
class _AddQuizScreenState extends ConsumerState<AddQuizScreen> {
  ...
  Future<void> _saveTopic() async {
    if (_topicNameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please enter a Topic Name")));
      return;
    }

    if (_levels.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please add at least one Level")));
      return;
    }
    ...
    if (adminId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Admin ID missing. Re-login.")));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(quizViewModelProvider, (previous, next) {
      if (next.isSuccess) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Topic '${_topicNameController.text}' Saved!"), backgroundColor: Colors.green),
        );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          "Create New Topic",
          style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 17.sp),
        ),
        centerTitle: true,
      ),
      body: Stack(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label: Text("Add Level", style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 13.sp)),
                ...
                if (_levels.isEmpty)
                  Container(
                    ...
                    child: Center(
                      child: Text("No levels added yet.\nClick 'Add Level' to start.",
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: quizState.isLoading
                        ...
                        : Text("SAVE TOPIC", style: GoogleFonts.poppins(fontSize: 15.sp, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: 1)),
                  ),
                ),
                SizedBox(height: 3.h),
              ],
  ...
  Widget _buildLabel(String text) {
    return Padding(
      ...
      child: Text(
        text.toUpperCase(),
        style: GoogleFonts.poppins(fontSize: 11.sp, fontWeight: FontWeight.w800, color: _textLabel, letterSpacing: 0.5),
      ),
    );
  }
  ...
  Widget _buildTextField({required TextEditingController controller, required String hint, required IconData icon, bool isNumber = false}) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _border, width: 1.5)),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _primary, width: 2.5)),
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text("Level ${level.order}: ${level.title}", style: GoogleFonts.poppins(fontSize: 14.sp, fontWeight: FontWeight.w700, color: _textDark)),
              Row(
                children: [
                  IconButton(
          ...
          Row(
            ...
            children: [
              ...
              _buildInfoBadge(Icons.percent, "${level.passingPercentage}% Pass", Colors.blue), // Updated to show Pass %
              _buildInfoBadge(Icons.help_outline, "${level.questions.length} Qs", Colors.purple),
            ],
          )
        ],
      ),
  ...
  Widget _buildInfoBadge(IconData icon, String text, Color color) {
    return Row(
      children: [
        ...
        Text(text, style: GoogleFonts.poppins(fontSize: 12.sp, fontWeight: FontWeight.w600, color: Colors.grey.shade700)),
      ],
    );
  }
          ...
          return Container(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: 2.h),
                Text(existingLevel == null ? "Add Level" : "Edit Level", style: GoogleFonts.poppins(fontSize: 16.sp, fontWeight: FontWeight.w800)),
                SizedBox(height: 3.h),
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        // Level Title
                        TextField(controller: titleCtrl, decoration: const InputDecoration(labelText: "Level Title", border: OutlineInputBorder())),
                        SizedBox(height: 2.h),

                        // Order, Points, Pass Percentage Row
                        Row(
                          children: [
                            Expanded(child: TextField(controller: orderCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Order", border: OutlineInputBorder()))),
                            SizedBox(width: 2.w),
                            Expanded(child: TextField(controller: pointsCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Points", border: OutlineInputBorder()))),
                            SizedBox(width: 2.w),
                            // --- ADDED: PASS PERCENTAGE FIELD ---
                            Expanded(child: TextField(controller: passCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Pass %", border: OutlineInputBorder()))),
                          ],
                        ),
                        SizedBox(height: 3.h),
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            TextButton.icon(
                              ...
                              label: Text("Add Question", style: GoogleFonts.poppins(fontSize: 12.sp, fontWeight: FontWeight.w600)),
                            )
                          ],
                        ),

                        if (tempQuestions.isEmpty)
                          Center(child: Padding(padding: EdgeInsets.all(2.h), child: Text("Add at least one question", style: TextStyle(color: Colors.grey, fontSize: 12.sp))))
                        else
                          ListView.builder(
                            shrinkWrap: true,
                            physics: const NeverScrollableScrollPhysics(),
                            ...
                            itemBuilder: (c, i) {
                              ...
                              return ListTile(
                                ...
                                title: Text("Q${i+1}: ${q.question}", maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.poppins(fontSize: 13.sp, fontWeight: FontWeight.w600)),
                                trailing: IconButton(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(index == null ? "ADD LEVEL" : "UPDATE LEVEL", style: GoogleFonts.poppins(color: Colors.white, fontSize: 14.sp, fontWeight: FontWeight.bold)),
                  ),
                )
              ],
            ),
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  TextField(onChanged: (v) => qText = v, decoration: const InputDecoration(hintText: "Question Text")),
                  SizedBox(height: 2.h),
                  ...
                  InkWell(
                    ...
                    child: Container(
                      height: 10.h,
                      width: double.infinity,
                      decoration: BoxDecoration(color: Colors.grey.shade100, border: Border.all(color: _primary), borderRadius: BorderRadius.circular(8), image: qImage != null ? DecorationImage(image: FileImage(qImage!), fit: BoxFit.cover) : null),
                      child: qImage == null ? Center(child: Text("Tap to add Image (Optional)", style: GoogleFonts.poppins(fontSize: 12.sp, color: _primary))) : null,
                    ),
                  ),
                  SizedBox(height: 2.h),
                  ...
                  ...List.generate(4, (i) {
                    ...
                    return RadioListTile(
                      ...
                      value: i, groupValue: selectedOptionIndex, onChanged: (v) => setState(() => selectedOptionIndex = v!),
                    );
                  }),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text("SAVE QUESTION", style: GoogleFonts.poppins(color: Colors.white)),
                    ),
                  )
                ],
              ),
